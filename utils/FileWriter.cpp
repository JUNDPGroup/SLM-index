...
//     write.open((filename + folder + expRecorder.distribution + "_" + to_string(expRecorder.dataset_cardinality) + "_" + to_string(expRecorder.skewness) + "_" + to_string(expRecorder.selectivity) + ".csv"), ios::out);
//     for (Mbr mbr : mbrs)
//     {
//         write << mbr.get_self();
//     }
...
void FileWriter::write_mbrs(vector<Mbr> mbrs, ExpRecorder expRecorder)
{
    ofstream write;
    string folder = Constants::WINDOW;
    string D = filename + folder + "D_" + to_string(expRecorder.dim)+ "/"; 
    file_utils::check_dir(D);
    // write.open((filename + folder + expRecorder.distribution + "_" + to_string(expRecorder.dataset_cardinality) + "_" + to_string(expRecorder.skewness) + "_" + to_string(expRecorder.window_size) + "_" + to_string(expRecorder.window_ratio) + ".csv"), ios::out);
    write.open((D + expRecorder.distribution + "_" + to_string(expRecorder.dataset_cardinality) + "_" + to_string(expRecorder.skewness) + "_" + to_string(expRecorder.selectivity) + ".csv"), ios::out);
    for (Mbr mbr : mbrs)
    {
        write << mbr.get_self();
    }
...
void FileWriter::write_build(ExpRecorder expRecorder)
{
    ...
    string file_path = filename + folder + "/" + expRecorder.structure_name + "_" + expRecorder.distribution + "_" + to_string(expRecorder.dataset_cardinality) + "_" + to_string(expRecorder.skewness) 
    + "/" + "D_" + to_string(expRecorder.dim) + "/" + "K_" + to_string(expRecorder.K) + "_B_" + to_string(expRecorder.B);
    file_utils::check_dir(file_path);
    string file_name = file_path + "/" ;
    write.open((file_name + + "N_" + to_string(expRecorder.N) + ".txt"), ios::app);
...
void FileWriter::write_baseline_point_query(ExpRecorder expRecorder)
{
    ofstream write;
    string folder = Constants::POINT;
    string file_path = filename + folder + "/"+ "baseline/" + expRecorder.structure_name + "_" + expRecorder.distribution + "_" + to_string(expRecorder.dataset_cardinality) + "_" + to_string(expRecorder.skewness) 
    + "/" + "K_" + to_string(expRecorder.K) + "_B_" + to_string(expRecorder.B);
    file_utils::check_dir(file_path);
    string file_name = file_path + "/" ;
    write.open((file_name + + "N_" + to_string(expRecorder.N) + ".txt"), ios::app);
...
void FileWriter::write_secure_point_query(ExpRecorder expRecorder)
{
    ofstream write;
    string folder = Constants::POINT;
    file_utils::check_dir(filename + folder);
    write.open((filename + folder + expRecorder.structure_name + "_" + expRecorder.distribution + "_" + to_string(expRecorder.dataset_cardinality) + "_" + to_string(expRecorder.skewness) + "_secure" + "_" + to_string(expRecorder.N)+ "_" + to_string(expRecorder.B) + "_" + to_string(expRecorder.K) + ".txt"), ios::app);
    write << expRecorder.get_time_pageaccess();
    write.close();
}
...
void FileWriter::write_baseline_window_query(ExpRecorder expRecorder)
{
    // ofstream write;
    // string folder = Constants::WINDOW + "Baseline/";
    // file_utils::check_dir(filename + folder);
    // write.open((filename + folder + expRecorder.structure_name + "_" + expRecorder.distribution + "_" + to_string(expRecorder.dataset_cardinality) + "_" + to_string(expRecorder.skewness) + "/" + "K_" + to_string(expRecorder.K) + "_B_" + to_string(expRecorder.B) +"_" + to_string(expRecorder.dim) + "_" + to_string(expRecorder.N) + ".txt"), ios::app);
    // write << expRecorder.get_time_pageaccess_accuracy();
    // write.close();
    ofstream write;
    string folder = Constants::WINDOW;
    string file_path = filename + folder + "/"+ "baseline/" + expRecorder.structure_name + "_" + expRecorder.distribution + "_" + to_string(expRecorder.dataset_cardinality) + "_" + to_string(expRecorder.skewness) 
    + "/" + "K_" + to_string(expRecorder.K) + "_B_" + to_string(expRecorder.B) + "_D_" + to_string(expRecorder.dim);
    file_utils::check_dir(file_path);
    string file_name = file_path + "/" ;
    write.open((file_name + + "N_" + to_string(expRecorder.N) + ".txt"), ios::app);
    write << expRecorder.get_time_pageaccess_accuracy();
...
void FileWriter::write_secure_window_query(ExpRecorder expRecorder)
{
    // ofstream write;
    // string folder = Constants::WINDOW + "Secure/";
    // file_utils::check_dir(filename + folder);
    // write.open((filename + folder + expRecorder.structure_name + "_" + expRecorder.distribution + "_" + to_string(expRecorder.dataset_cardinality) + "_" + to_string(expRecorder.skewness) + "_" + to_string(expRecorder.dim) + "_" + to_string(expRecorder.N) + ".txt"), ios::app);
    // write << expRecorder.get_time_pageaccess_accuracy();
    // write.close();
    ofstream write;
    string folder = Constants::WINDOW;
    string file_path = filename + folder + "/"+ "secure/" + expRecorder.structure_name + "_" + expRecorder.distribution + "_" + to_string(expRecorder.dataset_cardinality) + "_" + to_string(expRecorder.skewness) 
    + "/" + "K_" + to_string(expRecorder.K) + "_B_" + to_string(expRecorder.B) + "_D_" + to_string(expRecorder.dim);
    file_utils::check_dir(file_path);
    string file_name = file_path + "/" ;
    write.open((file_name + + "N_" + to_string(expRecorder.N) + ".txt"), ios::app);
    write << expRecorder.get_enc_depth_time_pageaccess_accuracy();
    write.close();
}
