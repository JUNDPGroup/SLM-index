...
struct enc_sortForKNN2
{
    // Paillier& paillier;  // 改为存储 const 引用

    // enc_sortForKNN2(Paillier& p) : paillier(p) {}  // 构造函数接受 const 引用

    bool operator()(Point point1, Point point2)  // 参数改为 const 引用
    {
        return SICrun(
            point1.enc_temp_dist,
            point2.enc_temp_dist,
            paillier  // 直接使用引用，无拷贝
        ) == 0;
    }
};
// struct enc_sortForKNN2 {
//     Paillier* paillier_ptr;  // 改用指针
//     enc_sortForKNN2(Paillier& pall) : paillier_ptr(&pall) {}  // 存储指针
    
//     bool operator()(Point& a, Point& b) const {
//         // 通过指针访问 Paillier
//         return (SICrun(
//             a.enc_temp_dist.get_mpz_t(),
//             b.enc_temp_dist.get_mpz_t(),
//             *paillier_ptr  // 解引用指针
//         ) == 0);
//     }
// };
